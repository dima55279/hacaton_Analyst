...
def create_dashboard_app(appeals_system):
    ...
    def get_stats():
        try:
            ...
            if not stats:
                ...
                stats = [
                    {'type': 'жалоба на ЖКХ', 'status': 'новое', 'count': 5},
                    {'type': 'предложение по благоустройству', 'status': 'отвечено', 'count': 3},
                    {'type': 'запрос информации', 'status': 'в работе', 'count': 2},
                    {'type': 'жалоба на дороги', 'status': 'новое', 'count': 4},
                    {'type': 'другое', 'status': 'требует проверки', 'count': 1}
                ]
    ...
    # НОВЫЕ ENDPOINTS ДЛЯ МУНИЦИПАЛИТЕТОВ
    @app.route('/api/municipality_stats')
    def get_municipality_stats():
        """Статистика по муниципалитетам"""
        try:
            period = request.args.get('period', 30, type=int)
            logger.info(f"🏛️ Запрос статистики по муниципалитетам за {period} дней")
            stats = system.database.get_municipality_stats(period)
            
            # Если статистика пустая, возвращаем демо-данные
            if not stats:
                logger.warning("🏛️ Статистика по муниципалитетам пустая, возвращаем демо-данные")
                stats = [
                    {
                        'municipality': 'Городской округ город Тамбов',
                        'appeal_count': 8,
                        'answered_count': 6,
                        'new_count': 2,
                        'in_progress_count': 0,
                    ...
                    {
                        'municipality': 'Мичуринский район',
                        'appeal_count': 5,
                        'answered_count': 3,
                        'new_count': 1,
                        'in_progress_count': 1,
            ...
            logger.info(f"🏛️ Возвращаем статистику по {len(stats)} муниципалитетам")
            return jsonify(stats)
            
        except Exception as e:
            logger.error(f"❌ Ошибка получения статистики по муниципалитетам: {e}")
            return jsonify({"error": "Ошибка получения статистики по муниципалитетам"}), 500

    @app.route('/api/municipality_type_stats')
    def get_municipality_type_stats():
        """Статистика по типам обращений в муниципалитетах"""
        try:
            period = request.args.get('period', 30, type=int)
            logger.info(f"🏛️ Запрос статистики по типам обращений за {period} дней")
            stats = system.database.get_municipality_type_stats(period)
            
            # Если статистика пустая, возвращаем демо-данные
            if not stats:
                logger.warning("🏛️ Статистика по типам обращений пустая, возвращаем демо-данные")
                stats = [
                    {
                        'municipality': 'Городской округ город Тамбов',
                        'appeal_type': 'жалоба на дороги',
                        'type_count': 4
                    },
                    {
                        'municipality': 'Городской округ город Тамбов',
                        'appeal_type': 'жалоба на ЖКХ',
                        'type_count': 3
                    },
                    {
                        'municipality': 'Городской округ город Тамбов',
                        'appeal_type': 'предложение по благоустройству',
                        'type_count': 1
                    },
                    {
                        'municipality': 'Мичуринский район',
            ...
            logger.info(f"🏛️ Возвращаем статистику по типам: {len(stats)} записей")
            return jsonify(stats)
            
        except Exception as e:
            logger.error(f"❌ Ошибка получения статистики по типам обращений: {e}")
            return jsonify({"error": "Ошибка получения статистики по типам обращений"}), 500
